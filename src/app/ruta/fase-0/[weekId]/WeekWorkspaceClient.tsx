...
import { ArrowLeft, Clock, CheckCircle2, Lock, Unlock, Play, ShieldAlert, Award, FileText, Send, Sparkles, BookOpen, HelpCircle, FlaskConical, ChevronRight, X, AlertCircle, Upload, Trash2, Mic, MicOff, Quote, PlusCircle, Volume2 } from 'lucide-react';
...
export default function WeekWorkspaceClient({ week }: { week: WeekData }) {
  ...
  // Función para síntesis de voz en español (Speech Synthesis TTS)
  ...
  const handleInsertPdfQuoteToDeliverable = () => {
    ...
    setPdfQuoteText('');
    setQuoteInsertedToast(true);
    setTimeout(() => setQuoteInsertedToast(false), 3000);
  };

  // Función para pegar automáticamente el texto copiado desde el portapapeles (1-Clic)
  const handlePasteClipboardQuote = async () => {
    try {
      if (typeof window === 'undefined' || !navigator.clipboard) return;
      const text = await navigator.clipboard.readText();
      if (text && text.trim()) {
        setPdfQuoteText(text.trim());
        const formattedQuote = `\n\n> 📑 **Cita Extraída (${pdfFileName || 'PDF'}):**\n> "${text.trim()}"\n`;
        setDeliverableContent((prev) => prev + formattedQuote);
        setQuoteInsertedToast(true);
        setTimeout(() => setQuoteInsertedToast(false), 3000);
      ...
      alert('Para pegar con 1-clic autoriza el permiso de portapapeles o usa Ctrl+V en la casilla.');
    }
  };
